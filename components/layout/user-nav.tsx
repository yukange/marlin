...
export function UserNav() {
  ...
  const handleSyncCurrentSpace = async () => {
    if (!currentSpace || !githubUser?.login || isSyncing) return

    setSpaceSyncStatus(currentSpace, 'syncing')
    try {
      const { uploaded, downloaded, pruned, conflicts } = await syncWorkspace(currentSpace, githubUser.login)

      if (uploaded === 0 && downloaded === 0 && pruned === 0 && conflicts === 0) {
        toast.success('Already up to date')
      } else {
        const messages: string[] = []
        if (uploaded > 0) messages.push(`${uploaded} uploaded`)
        if (downloaded > 0) messages.push(`${downloaded} downloaded`)
        if (pruned > 0) messages.push(`${pruned} deleted`)
        if (conflicts > 0) {
          messages.push(`${conflicts} conflicts`)
          toast.warning(`Synced with conflicts: ${messages.join(', ')}. Check notes tagged with #conflict`)
        } else {
          ...
          <Button variant="ghost" className="h-auto w-full justify-start p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 focus-visible:ring-0 focus-visible:ring-offset-0">
            <div className="relative">
              <Avatar className="h-10 w-10 rounded-md">
                ...
                <AvatarFallback className="rounded-md">{sessionUser?.name?.[0]}</AvatarFallback>
              </Avatar>
              <div className={cn(
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              Logging out will delete all local data, and these changes will be lost forever.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
