...
function determinePlan(product?: { id?: string; name?: string }): 'monthly' | 'yearly' | 'lifetime' {
    ...
    if (productId.includes('yearly') || productId.includes('annual') ||
        productName.includes('yearly') || productName.includes('annual')) {
        return 'yearly'
    }
    return 'monthly'
}
...
export const POST = Webhook({
     ...
     */
    onGrantAccess: async ({ customer, metadata, product, id: subscriptionId, current_period_end_date }: GrantAccessContext) => {
     ...
     */
    onSubscriptionCanceled: async ({ metadata }: FlatSubscriptionEvent<'subscription.canceled'>) => {
        const githubId = metadata?.referenceId as string | undefined
        if (githubId) {
     ...
     */
    onSubscriptionExpired: async ({ metadata }: FlatSubscriptionEvent<'subscription.expired'>) => {
        const githubId = metadata?.referenceId as string | undefined
        if (githubId) {
            await revokeLicense(githubId)
        }
     ...
     */
    onSubscriptionPaused: async ({ metadata }: FlatSubscriptionEvent<'subscription.paused'>) => {
        const githubId = metadata?.referenceId as string | undefined
        if (githubId) {
            await revokeLicense(githubId)
        }
    ...
    onRefundCreated: async (data: FlatRefundCreated) => {
        ...
        if (githubId) {
            ...
            console.error('[webhook] onRefundCreated: Cannot find referenceId in subscription or checkout metadata')
        }
    },
})
