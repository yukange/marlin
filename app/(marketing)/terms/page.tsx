...
export default async function TermsPage() {
  const fileContent = termsContent;

  // Parse metadata from markdown
  const lines = fileContent.split("\n");
  const titleLine = lines.find((line: string) => line.startsWith("# "));
  const title = titleLine?.replace("# ", "") || "Terms of Service";

  const lastUpdatedLine = lines.find((line: string) =>
    line.includes("Last Updated")
  );
  const lastUpdated =
    lastUpdatedLine?.replace(/\*\*Last Updated:\*\*\s*/, "") || "";
